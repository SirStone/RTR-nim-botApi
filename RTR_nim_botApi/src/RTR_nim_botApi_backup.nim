...
var botRun = false
var sendFlag = false
var inputBM, outputBM: seq[string] = @[]

proc prepend[T](repo: var seq[T], item:T) = 
  repo = item & repo

proc pop[T](repo: var seq[T]):T = 
  if repo.len > 0:
    let item = repo.pop()
    return item
  else:
...
proc messageListener() {.async.} =
  # while the connection is open...
  while(gs_ws.readyState == Open):
    echo "MessageListener checking message"
    ...
    inputBM.prepend(json_message)

proc messageSender() {.async.} =
  # while the connection is open...
  while(gs_ws.readyState == Open):
    echo "MessageSender checking for messages to send"
    if outputBM.len > 0:
      # get the message from the output Message Buffer
      let message = outputBM.pop()
      echo "sending message: ", message
      # send the message
      waitFor gs_ws.send(message)
    else:
